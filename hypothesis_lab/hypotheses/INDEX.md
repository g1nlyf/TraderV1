...
| H-166 | Exit-overlay risk module (distribution → exit) | **tested · DEMOTED (not validated)** | exit_h166 −12.84% beats hold/random-time but LOSES to random-sell −11.69%; veto worthless | — | 1137 | 2026-06-06 | Sprint 7: built deterministic module (8/8 fixtures) + backtest vs controls. Quality-distribution specificity adds nothing over exit-on-any-sell. Research curiosity/1-session. SHADOW-only. H166_RISK_ENGINE_REPORT.md |
| H-167 | Distributor archetype (repeatedly sells before drop) | **tested · REAL but < token-only** | rho +0.262, top-half SHORT +25.45% vs base +17.57% | — | 390 | 2026-06-06 | Best WALLET feature, point-in-time. Beaten by tok_prior_ret (+0.332). Keep for H-163 cross-day only. WALLET_DISTRIBUTION_THEORY.md |
| H-168 | Co-sell network / cabal cohesion | **tested · DEAD** | cohesion rho −0.102 (co-sellers don't out-drop) | — | 390 | 2026-06-06 | Coordinated co-sell does NOT predict bigger drops. Cabal-distribution theory refuted on this data. WALLET_DISTRIBUTION_THEORY.md |
| H-170 | Token lifecycle state machine | **tested · REAL separator, not incremental** | neutral-state edge +8.0% over base, perm 0.000; but state one-hot adds +0.10% over continuous token feats | — | 1027 | 2026-06-06 | States separate forward EV (avoidance axis, all negative). GBM already captures lifecycle from raw feats. Value = interpretable risk states + +2.3% no-trade avoidance filter (shadow). Not alpha. H170_TOKEN_LIFECYCLE_REPORT.md |
| H-171 | Wallet adds incremental multivariate OOS value | **tested · CONFIRMED (walk-forward)** | token+wallet +1.78% over token-only (perm 0.000, CI upper +2.14% via token+wq); carried by BOTH wq & cohesion | — | 1137 | 2026-06-06 | Sprint-9 corrected walk-forward CONFIRMS Sprint-8 (revised wallet-dead). H-171b ablation: increment distributed across wq+cohesion. Best ranker yet. Still neg EV. VALIDATION_AUDIT.md, TOURNAMENT_REPORT.md |
| H-184 | Rug pre-detection no-trade filter | **tested · ROBUST de-risk (shadow)** | GBM P(net<−50%) skip top-30% risk → edge +4.73% over base, perm 0.000, n=431 | — | 431 | 2026-06-06 | Correct way to do avoidance (naive state-skip was n.s. 0.117). Capturable no-trade filter; rugs gap to ~0 (exit can't fix) → avoid pre-entry. Still EV<0 (down-regime). Shadow. tournament.py |
| H-180..H-191 | Compound hypothesis queue (archetype-consensus, wallet-token fit, lifecycle continuation/reversal, liquidity migration, co-buy cluster, sizing overlay, regime tagger, edge stack) | **queued** | — | — | — | 2026-06-06 | 20+ compound hypotheses, clustered/prioritized, 12 test-now. HYPOTHESIS_QUEUE.md. Loop: tournament.py registry → walk-forward gate → ledger |
